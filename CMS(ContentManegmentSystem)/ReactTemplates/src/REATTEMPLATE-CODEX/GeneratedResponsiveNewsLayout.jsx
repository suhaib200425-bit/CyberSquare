...
export const responsiveNewsLayoutStyleConfig = {
  pageBackgroundColor: {
...
export const responsiveNewsLayoutContentData = {
  categoriesHeading: {
...
function useResponsiveNewsViewport() {
  const [viewport, setViewport] = useState({ isMobile: false, isTablet: false })

  useEffect(() => {
    const updateViewport = () => {
      setViewport({ isMobile: window.innerWidth < 768, isTablet: window.innerWidth < 1180 })
    }
    updateViewport()
    window.addEventListener('resize', updateViewport)
    return () => window.removeEventListener('resize', updateViewport)
...
function SectionHeading({ title, styleConfig, isMobile, trailingContent }) {
  return (
    <div className="w-full">
      <div
        className="flex w-full flex-col md:flex-row md:items-start md:justify-between"
        style={{ gap: styleConfig.articleRowGapDesktop.value }}
      >
        <div className="w-full">
          <h2
            style={{
              color: styleConfig.sectionTitleColor.value,
              fontSize: isMobile
                ? styleConfig.sectionTitleFontSizeMobile.value
                : styleConfig.sectionTitleFontSizeDesktop.value,
              fontWeight: styleConfig.sectionTitleFontWeight.value,
              lineHeight: styleConfig.titleLineHeight.value,
              margin: 0,
            }}
          >
            {title.value}
          ...
          <div
            style={{
              position: 'relative',
              width: '100%',
              height: styleConfig.headingUnderlineHeight.value,
              backgroundColor: styleConfig.headingUnderlineColor.value,
              marginTop: styleConfig.badgeGap.value,
            }}
          >
            <span
              style={{
                ...
                top: 0,
                height: styleConfig.headingUnderlineHeight.value,
                width: styleConfig.headingUnderlineAccentWidth.value,
                backgroundColor: styleConfig.headingUnderlineAccentColor.value,
              }}
            />
          </div>
        </div>
...
function BadgeList({ badges, styleConfig }) {
  return (
    <div className="flex flex-wrap" style={{ gap: styleConfig.badgeGap.value }}>
      {badges.value.map((badge) => (
        <span
          key={`${badge.label}-${badge.value}`}
          style={{
            backgroundColor: styleConfig.badgeBackgroundColor.value,
            borderRadius: styleConfig.badgeRadius.value,
            color: styleConfig.badgeTextColor.value,
            fontSize: styleConfig.badgeFontSize.value,
            fontWeight: styleConfig.tabFontWeight.value,
            padding: styleConfig.badgePadding.value,
          }}
        >
          {badge.value}
        </span>
...
function MetaRow({ items, styleConfig, overlay }) {
  return (
    <div
      className="flex flex-wrap items-center"
      style={{ gap: styleConfig.tabsGap.value, marginTop: styleConfig.badgeGap.value }}
    >
      {items.map((item) => (
        <span
          key={item}
          className="inline-flex items-center"
          style={{
            gap: styleConfig.badgeGap.value,
            color: overlay
              ? styleConfig.overlayMetaTextColor.value
              : styleConfig.metaTextColor.value,
            fontSize: styleConfig.metaFontSize.value,
            fontWeight: styleConfig.metaFontWeight.value,
          }}
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
            style={{ width: styleConfig.iconSize.value, height: styleConfig.iconSize.value }}
          >
            <path
              d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm7 8a7 7 0 0 0-14 0"
              stroke="currentColor"
...
function OverlayCard({
  ...
  return (
    <article
      style={{
        position: 'relative',
        minHeight: isMobile ? heightMobile.value : heightDesktop.value,
        overflow: 'hidden',
        borderRadius: styleConfig.cardRadius.value,
      }}
    >
      <img
        src={article.value.imageUrl.value}
        alt={article.value.imageAlt.value}
        style={{
          position: 'absolute',
          inset: 0,
          width: '100%',
          height: '100%',
          objectFit: styleConfig.imageObjectFit.value,
        }}
      />
      <div
        style={{ position: 'absolute', inset: 0, background: styleConfig.overlayGradient.value }}
      />
      <div
        style={{
          position: 'absolute',
          ...
          justifyContent: 'space-between',
          padding: isMobile
            ? styleConfig.overlayPaddingMobile.value
            : styleConfig.overlayPaddingDesktop.value,
        }}
      >
        <BadgeList badges={article.value.badges} styleConfig={styleConfig} />
        <div>
          <h3
            style={{
              color: styleConfig.overlayTitleColor.value,
              fontSize: isMobile ? titleSizeMobile.value : titleSizeDesktop.value,
              fontWeight: styleConfig.titleFontWeight.value,
              lineHeight: styleConfig.titleLineHeight.value,
              margin: 0,
            }}
          >
          ...
          <MetaRow
            items={[article.value.author.value, article.value.date.value]}
...
function ArticleRow({ article, styleConfig, isMobile }) {
  return (
    <article
      className="flex"
      style={{
        gap: isMobile
          ? styleConfig.articleRowGapMobile.value
          : styleConfig.articleRowGapDesktop.value,
      }}
    >
      <img
        ...
        style={{
          width: isMobile
            ? styleConfig.articleImageWidthMobile.value
            : styleConfig.articleImageWidthDesktop.value,
          height: isMobile
            ? styleConfig.articleImageHeightMobile.value
            : styleConfig.articleImageHeightDesktop.value,
          objectFit: styleConfig.imageObjectFit.value,
          borderRadius: styleConfig.cardRadius.value,
          flex: '0 0 auto',
        }}
      />
      <div className="flex min-w-0 flex-col justify-center">
        <BadgeList badges={article.badges} styleConfig={styleConfig} />
        <h3
          style={{
            color: styleConfig.titleColor.value,
            fontSize: isMobile
              ? styleConfig.articleTitleFontSizeMobile.value
              : styleConfig.articleTitleFontSizeDesktop.value,
            fontWeight: styleConfig.titleFontWeight.value,
            lineHeight: styleConfig.titleLineHeight.value,
            margin: `${styleConfig.badgeGap.value} 0 0`,
          }}
        >
          {article.title.value}
        </h3>
        <MetaRow items={[article.author.value, article.date.value]} styleConfig={styleConfig} />
      </div>
    </article>
  )
}

function PopularCompactItem({ item, styleConfig }) {
  return (
    <article className="flex" style={{ gap: styleConfig.articleRowGapDesktop.value }}>
      <img
        src={item.imageUrl.value}
        alt={item.imageAlt.value}
        style={{
          width: styleConfig.popularThumbWidth.value,
          height: styleConfig.popularThumbHeight.value,
          objectFit: styleConfig.imageObjectFit.value,
          borderRadius: styleConfig.cardRadius.value,
          flex: '0 0 auto',
        }}
      ...
      <div className="flex min-w-0 flex-col justify-center">
        <h3
          style={{
            color: styleConfig.titleColor.value,
            fontSize: styleConfig.popularTitleFontSize.value,
            fontWeight: styleConfig.titleFontWeight.value,
            lineHeight: styleConfig.titleLineHeight.value,
            margin: 0,
          }}
        ...
        <MetaRow items={[item.author.value]} styleConfig={styleConfig} />
      </div>
    </article>
  )
}

export function ResponsiveNewsLayoutModular({
  styleConfig = responsiveNewsLayoutStyleConfig,
  contentData = responsiveNewsLayoutContentData,
}) {
  const { isMobile, isTablet } = useResponsiveNewsViewport()

  return (
    <section
      className="w-full"
      style={{
        backgroundColor: styleConfig.pageBackgroundColor.value,
        padding: isMobile ? styleConfig.mobilePadding.value : styleConfig.desktopPadding.value,
    ...
    >
      <div
        className="mx-auto w-full"
        style={{
          maxWidth: styleConfig.containerMaxWidth.value,
          display: 'grid',
          gridTemplateColumns: isTablet
            ? styleConfig.tabletColumnTemplate.value
            : styleConfig.desktopColumnTemplate.value,
          gap: isMobile ? styleConfig.mobileMainGap.value : styleConfig.desktopMainGap.value,
        }}
      >
        <div>
          <SectionHeading
            title={categoriesHeading}
            styleConfig={styleConfig}
            isMobile={isMobile}
            trailingContent={
              <div
                className="flex flex-wrap md:justify-end"
                style={{
                  gap: styleConfig.tabsGap.value,
                  marginTop: isMobile ? styleConfig.badgeGap.value : '0px',
                }}
              >
                {tabs.value.map((tab, index) => (
                  <button
                    ...
                    style={{
                      ...
                      color:
                        index === 0
                          ? styleConfig.tabActiveTextColor.value
                          : styleConfig.tabTextColor.value,
                      cursor: 'pointer',
                      fontSize: styleConfig.tabFontSize.value,
                      fontWeight: styleConfig.tabFontWeight.value,
                      padding: 0,
                      textTransform: 'uppercase',
                    }}
          ...
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: isMobile
                ? styleConfig.categoryMobileTemplate.value
                : styleConfig.categoryDesktopTemplate.value,
              gap: isMobile ? styleConfig.mobileMainGap.value : styleConfig.articleStackGap.value,
              marginTop: styleConfig.headingBottomMargin.value,
            }}
          >
            <OverlayCard
              ...
              styleConfig={styleConfig}
              isMobile={isMobile}
              heightDesktop={styleConfig.featuredCardHeightDesktop}
              heightMobile={styleConfig.featuredCardHeightMobile}
              titleSizeDesktop={styleConfig.featuredTitleFontSizeDesktop}
              titleSizeMobile={styleConfig.featuredTitleFontSizeMobile}
            />
            <div className="flex flex-col" style={{ gap: styleConfig.articleStackGap.value }}>
              {categoryArticles.value.map((article, index) => (
                <ArticleRow
                  key={`${article.title.value}-${index}`}
                  article={article}
                  styleConfig={styleConfig}
                  isMobile={isMobile}
                />
        ...
        <aside>
          <SectionHeading
            ...
            isMobile={isMobile}
          />
          <div style={{ marginTop: styleConfig.headingBottomMargin.value }}>
            <OverlayCard
              article={popularHero}
              styleConfig={styleConfig}
              isMobile={isMobile}
              heightDesktop={styleConfig.popularHeroHeightDesktop}
              heightMobile={styleConfig.popularHeroHeightMobile}
              titleSizeDesktop={styleConfig.popularTitleFontSize}
              titleSizeMobile={styleConfig.popularTitleFontSize}
            />
          </div>
          <div
            className="flex flex-col"
            style={{
              gap: styleConfig.popularListGap.value,
              marginTop: styleConfig.popularListGap.value,
            }}
          >
            {popularItems.value.map((item, index) => (
              <PopularCompactItem
...
export default ResponsiveNewsLayoutModular
